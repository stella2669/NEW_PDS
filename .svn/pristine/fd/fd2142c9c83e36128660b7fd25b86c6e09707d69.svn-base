...
var iterationCnt = 204;
/*
 * These are the functions you'll usually want to call
 * They take string arguments and return either hex or base-64 encoded strings
 */

function getCrypto(str, cnt){

  if (str.length < 7 ) {
    return "error";
  }
  
  var slat = str.substring(2,3);  
  slat = base64enco(slat);
  
  var rn_rstr_sha256 = str2rstr_utf8( slat + str + slat );

  for (var i=0; i< cnt; i++) {
    rn_rstr_sha256 = rstr_sha256(rn_rstr_sha256);
  }
  
  return base64enco(rn_rstr_sha256); // base64
}
...
function b64_sha256(s) { 
  return base64enco(rstr_sha256(str2rstr_utf8(s))); 
}
*/
...
function b64_hmac_sha256(k, d) {
  return base64enco(rstr_hmac_sha256(str2rstr_utf8(k), str2rstr_utf8(d))); 
}
