...
$("#search").click(function(){
	...
	$("#ifr_tabs-PG_ALCA_01", parent.document).attr("src", queryString);
});
...
$("#excelDown").click(function(){
	var title="";
	var fileNm="";
	var url = $("#ifr_tabs-PG_ALCA_01", parent.document).attr("src");
	if(schby=="DEFAULT"){
		title="캠페인 요약통계";
		fileNm="캠페인 요약통계_"+schStrtDate+"_"+schEndDate;
	}else if(schby=="DATE"){
		title="캠페인 일자별통계";
		fileNm="캠페인 일자별통계_"+schStrtDate+"_"+schEndDate;
	}else{
		title="캠페인 시간대별통계";
		fileNm="캠페인 시간대별통계_"+schStrtDate+"_"+schEndDate;
	}
	url = "/al/alca01ex.do?"+url.split("?")[1]+"&title="+title+"&fileNm="+fileNm;
